...
| 你想做什么 | 去哪里 |
|------------|--------|
| 安装 APP 测试 | 下载 `apk/VisionGuard-v1.5.4-local.apk`（连本地）或 `submission/android/apk/VisionGuard-v1.5.4-cloud.apk`（连云服务器） |
...
│
├── apk/                            # 📱 发布 APK（直接安装）
│   └── VisionGuard-v1.5.4-local.apk # ★ 本地版 (127.0.0.1:3000)
│
...
│   ├── android/
│   │   └── apk/VisionGuard-v1.5.4-cloud.apk  # ★ 云版 (47.94.146.53/vg)
│   ├── hardware/                   #    硬件固件副本
...
| 代码审查 | ✅ | 4 轮审查，61 检查点通过 |
| Android 开发 | ✅ | 18 页面 + 全局下拉刷新 + 核心流程贯通 + 真机验证通过，v1.5.4 APK 已签名 |
